...
echo "--------------------------------------------------------------------------------------"
echo "------------------ RMX1805 Kernel - Build Script -------------------"
echo "--------------------------------------------------------------------------------------"
...
if [ ! -d clang ]
then
git clone --depth=1 https://github.com/crdroidandroid/android_prebuilts_clang_host_linux-x86_clang-5407736 clang
...
export ARCH=arm64
export KBUILD_BUILD_USER=abhinav
...
function zipping() {
    ...
    cd AnyKernel || exit 1
    zip -r9 Stock-RMX1805-${TANGGAL}.zip * > /dev/null 2>&1
    cd ..
    echo "Zip stored at AnyKernel/Stock-RMX1805-${TANGGAL}.zip"
    echo -e "${RST}"
}
compile
